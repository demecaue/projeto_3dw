...
const ROTULO_ESTOQUE: Record<string, string> = {
    CRITICO: 'Crítico',
    ATENCAO: 'Atenção',
    OK: 'Normal'
};

function rotuloEstoque(situacao: string): string {
...
function exibir(id: string, visivel: boolean): void {
    const alvo = elemento(id);
    if (alvo) {
...
function renderizarPeriodo(lista: Aplicacao[]): void {
    ...
    escrever(
        'carimbo-periodo',
        `${formatarData(antigo.data_aplicacao)} a ${formatarData(recente.data_aplicacao)}`
    );
}

function renderizarTabela(lista: Aplicacao[]): void {
...
async function iniciar(manual: boolean): Promise<void> {
    if (manual) {
        botaoOcupado(true);
    } else {
