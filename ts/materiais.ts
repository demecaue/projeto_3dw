import type { Categoria, Material, MaterialForm } from './tipos.js';
...
// Desenvolvimento Web Avancada: tela do CRUD de materiais

const RECURSO = 'materiais';
let editandoId: number | null = null;

function renderizarTabela(lista: Material[]): void {
    ...
    for (const material of lista) {
        ...
        linha.append(
            celula('',
                criar('span', 'linha-titulo', material.nome),
                criar('span', 'linha-sub', material.categoria)
            ),
            celula('text-end num', `${formatarMoeda(material.valor_unitario)} / ${material.unidade}`),
            celula('text-end col-estoque',
                criar('span', 'linha-saldo num', `${material.estoque_atual} / mín. ${material.estoque_minimo}`),
                badgeEstoque(material.situacao_estoque)
            ),
            celula('text-end num', String(material.total_lancamentos)),
            botoesAcao(
                (): void => abrirFormulario(material),
                (): void => { void pedirExclusao(material); }
            )
        );
...
async function carregarCategorias(): Promise<void> {
    ...
    if (resultado.ok) {
        preencherSelect('campo-categoria', resultado.dados.map((categoria: Categoria): { valor: string; texto: string } => ({
...
async function carregar(): Promise<void> {
    ...
    const resultado = await listar<Material>(RECURSO);
...
function abrirFormulario(material: Material | null): void {
...
function lerFormulario(): MaterialForm {
    return {
...
async function enviarFormulario(): Promise<void> {
    ...
    const resultado = await salvar<MaterialForm>(RECURSO, dados, editandoId);
    if (!resultado.ok) {
...
async function pedirExclusao(material: Material): Promise<void> {
